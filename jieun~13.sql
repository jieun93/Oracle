...
1)CAST
 ...
 to_char(c1 [,fmt])
   ...
   ** 날짜 형식 지정 문자열
   ...
   Q                                  분기
   MONTH, MON,MM,RM                   월
   W, WW, IW                          주/*ww:1월1일부터 몇주차인지 알려주는것*/
   DD,DDD,J                           일/*ddd : 경과된 일수를 나타낼 때*/
   DAY, DY, D                         요일
   AM, PM, A.M.,P.M.                  오전/오후
   HH, HH12,HH24                      시간 
   MI                                분
   ...
   -----------------------------------------------------
    ...
    select to_char(to_date('20191220'),'yyyy-mm-dd am hh24:mi:ss w ddd dy')from dual;
    
    
    예) 장바구니 테이블에서 장바구니 번호(cart_no)를 사용하여 판매일 (월일)별 판매금액 합계를 구하시오. 
        단, 2005년 5월에 판매된 거래에 대하여 조회하시오.
        Alias는 판매일 ,  수량합계, 판매액합계
  ...
  **숫자형식지정문자열
  ---------------------------------------------------------
    형식문자열                   의미
  --------------------------------------------------------------------------------
      9                    숫자와 대응되어 유효숫자가 대응되는 위치의 '9': 유효숫자 반환   
                            '9'와 대응되는 데이터가 무효의 '0'이면 공백처리
                            ex) 형식지정 문자열: '999999'
                                데이터:         001234 --> 1234 출력
                                               010012 --> 10012 출력
                                               001000 --> 1000 출력
                                              1223456 --> 오류
 ----------------------------------------------------------------------------------
      0                   숫자와 대응되어 유효숫자가 대응되는 위치의 '0': 유효숫자 반환   
                            '0'와 대응되는 데이터가 무효의 '0'이면 0을 출력
                            ex) 형식지정 문자열: '000000'
                                데이터:         001234 --> 001234  출력
                                               010012 --> 010012 출력
                                               001000 --> 001000 출력
                                              1223456 --> 오 류
 ----------------------------------------------------------------------------------
    ,(comma).(period) 해당위치에 ,와 .을 삽입하여 반환
    $(L)              화폐기호($,로케이터에 등록된 화폐기호) 출력
    mi                음수인 경우 우측에 '-' 부호 출력
    pr                음수인 경우 '-' 대신 자료를 '< >'로 묶어 출력
 ----------------------------------------------------------------------------------
                            
    예) 상품테이블에서 상품 판매가격이 15만원 이상인 상품을 조회하시오.
        단, 가격 출력시 화폐기호와 자리점 (3자리마다 ',')을 넣어 출력할것
        Alias는 상품번호, 상품명, 가격
        
        select  prod_id as 상품번호,
                prod_name as  상품명,
               to_char(prod_price,'L9,999,999PR') as 가격1,
                to_char(prod_price,'L9,999,999') as 가격
        from prod
        where prod_ID<='P102';
